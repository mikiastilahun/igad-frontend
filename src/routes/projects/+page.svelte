...
<!-- <section class="mx-auto max-w-[1136px] px-4 py-10">
	<h2 class="text-2xl font-bold">Our Projects</h2>
	<div class="mt-8 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-2 lg:gap-6 2xl:gap-8">
		{#each projects as project}
			{@const coverImage = project?.attributes?.cover_image.data?.attributes.url}
			<div
				class="relative mb-8 h-full min-h-[420px] rounded-lg shadow lg:h-[320px] lg:min-h-[320px]"
			>
				<img
					src={`${coverImage ? `${PUBLIC_STATIC_URL}${coverImage}` : IGADLogo}`}
					...
					class="{coverImage
						? 'object-cover'
						: 'object-contain'} absolute inset-0 h-full w-full rounded-lg bg-primary-500"
				/>
				<div
					class="absolute inset-0 h-full w-full rounded-lg bg-gradient-to-t from-stone-950 from-40% to-transparent p-4"
				></div>
				<div class="relative flex h-full w-full flex-col items-end gap-4 text-white">
					<div class=" mx-4 mb-4 flex flex-1 flex-col flex-wrap justify-end gap-2 text-sm">
						<p class="">
							<span class=" inline-flex items-center gap-2 text-nowrap align-middle text-secondary">
								<svg
									xmlns="http://www.w3.org/2000/svg"
									width="16"
									height="16"
							...
							<span class="ml-2 align-middle">{project.attributes.implementing_organization}</span>
						</p>
						<p class="">
							<span class="inline-flex items-center gap-2 align-middle text-secondary"
								><svg
									xmlns="http://www.w3.org/2000/svg"
									width="16"
									height="16"
						...
						<div class="flex flex-col items-start justify-between gap-2 md:flex-row">
							...
							{#if project.attributes?.files.data?.attributes.url}
								<a
									href={`${PUBLIC_STATIC_URL}${project.attributes?.files.data?.attributes.url}`}
									target="_blank"
									class=" inline-block w-fit rounded bg-secondary px-4 py-2 text-sm text-stone-900 drop-shadow-lg md:text-base"
									>Download PDF</a
								>
							{/if}
						</div>
